...
class WhisperSubtitleExtractor:
    ...
    def extract_subtitles(self, audio_path, language=None, task="transcribe",
                         output_dir="output", output_formats=["srt"], word_timestamps=False):
        """
        提取字幕
    ...
    def batch_process(self, input_dir, language=None, task="transcribe",
                     output_dir="batch_output", output_formats=["srt"], word_timestamps=False):
        """
        批量处理目录中的音频文件
...
def main():
    ...
    parser.add_argument("--word-timestamps", action="store_true", help="提取单词级时间戳")
